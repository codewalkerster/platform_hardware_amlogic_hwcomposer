...
bool DisplayAdapterLocal::setWhiteBoardMode(bool mode, int callingPid){
    return MesonHwc2::getInstance().setWhiteBoardMode(mode, callingPid);
}

bool DisplayAdapterLocal::getWhiteBoardMode(bool& mode){
    MesonHwc2::getInstance().getWhiteBoardMode(mode);
    return true;
}
...
bool DisplayAdapterLocal::getHdrSdrRatio(float & ratio, ConnectorType displayType) {
    ...
    drm_hdr_capabilities caps;
    connector->getHdrCapabilities(&caps);
    ratio = caps.maxLuminance / 100.0;
    return true;
}
...
bool DisplayAdapterLocal::setDisplayRect(const Rect rect, ConnectorType displayType) {
    ...
    ret = MesonHwc2::getInstance().setViewPort(drm_rect, type);
...
bool DisplayAdapterLocal::getDisplayRect(Rect& rect, ConnectorType displayType) {
    ...
    GET_CRTC_BY_CONNECTOR(type);
    if (crtc) {
        MesonHwc2::getInstance().getViewPort(drm_rect, displayType);
